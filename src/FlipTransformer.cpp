...
uniform mat3 boxToClip;
uniform mat3 boxToSource; // card-local unit square -> source texture clip space
uniform vec3 rotation; // cosine, sine, bounded projection scale
uniform float perspective;
vec4 sampleFace(sampler2D face, vec2 p) {
    vec2 uv = (boxToSource * vec3(p * 0.5 + 0.5, 1.0)).xy * 0.5 + 0.5;
...
void main() {
    ...
    vec2 sampleUV = (boxToSource * vec3(point * 0.5 + 0.5, 1.0)).xy * 0.5 + 0.5;
...
bool FlipShader::initialize(std::string &error) {
    ...
    source = glGetUniformLocation(p, "boxToSource");
    return true;
}
...
CBox FlipTransformer::cardBox() const {
    const auto w = m_window.lock();
    const auto monitor = w ? w->m_monitor.lock() : nullptr;
    if (!w || !monitor)
        return {};
...
CBox FlipTransformer::transformedExtents(const CBox &currentBox) const {
    const auto card = cardBox();
    if (!m_active || card.empty())
        return currentBox;
    const double x = std::min(currentBox.x, card.x), y = std::min(currentBox.y, card.y);
    return {x, y, std::max(currentBox.x + currentBox.w, card.x + card.w) - x,
            std::max(currentBox.y + currentBox.h, card.y + card.h) - y};
}
// Hyprland hands us the window rendered into a canvas at in.box and expects an
// output canvas covering context.outputBox (both monitor-local pixels).
Render::SWindowTransformBuffer FlipTransformer::transform(const Render::SWindowTransformBuffer &in,
                                                          const Render::SWindowTransformContext &context) {
    const auto w = m_window.lock();
    const auto monitor = context.monitor.lock();
    if (!w || !monitor || !in.framebuffer || !in.framebuffer->getTexture() || m_pose->failed || context.standalone ||
        context.renderingSnapshot)
        return in;
    ...
    const auto canvas = Render::pixelBoxForLogical(context.outputBox.copy().expand(1.0 / monitor->m_scale), monitor->m_scale);
    if (canvas.empty())
        return in;
    if (snapshot && !m_pose->dirty && m_pose->composite && m_pose->compositeBox == canvas)
        return {.framebuffer = m_pose->composite, .box = canvas};
    GLState state;
    if (!m_shader->initialize(m_pose->error)) {
        m_pose->failed = true;
        return in;
    }
    if (snapshot && (!m_pose->composite || m_pose->compositeBox.size() != canvas.size())) {
        m_pose->composite = g_pHyprRenderer->createFB("Hyprflip transition");
        if (!m_pose->composite->alloc(canvas.w, canvas.h, DRM_FORMAT_ABGR8888)) {
            m_pose->error = "Could not allocate the card transition";
            m_pose->failed = true;
            return in;
        }
        m_pose->composite->setImageDescription(in.framebuffer->imageDescription());
    }
    auto out = snapshot ? m_pose->composite : monitor->resources()->getUnusedWorkBuffer(canvas.size());
    if (!out) {
    ...
    if (snapshot)
        m_pose->compositeBox = canvas;
    auto &render = g_pHyprRenderer->m_renderData;
    const auto oldFBSize = render.fbSize;
    const auto oldProjection = render.projectionType;
    const CRegion oldDamage = render.damage.copy();
    const CBox box = cardBox().scale(monitor->m_scale);
    // Snapshot faces are whole-monitor captures; live content is the window canvas.
    const CBox sourceSpace = snapshot ? CBox{0, 0, monitor->m_transformedSize.x, monitor->m_transformedSize.y} : in.box;
    g_pHyprRenderer->setProjectionType(Render::RPT_EXPORT);
    render.fbSize = sourceSpace.size();
    const auto toSource =
        g_pHyprRenderer->projectBoxToTarget(box.copy().translate(-sourceSpace.pos()), HYPRUTILS_TRANSFORM_NORMAL).copy().transpose();
    auto guard = g_pHyprRenderer->bindTempFB(out);
    render.fbSize = canvas.size();
    render.damage = CRegion{0, 0, canvas.w, canvas.h};
    const auto matrix =
        g_pHyprRenderer->projectBoxToTarget(box.copy().translate(-canvas.pos()), HYPRUTILS_TRANSFORM_NORMAL).copy().transpose();
    const auto composite = g_pHyprRenderer->projectBoxToTarget({0, 0, canvas.w, canvas.h}).copy().transpose();
    glDisable(GL_BLEND);
    glDisable(GL_SCISSOR_TEST);
    glDisable(GL_DEPTH_TEST);
    glDisable(GL_STENCIL_TEST);
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, (snapshot ? m_pose->faces[0] : in.framebuffer)->getTexture()->m_texID);
    glBindSampler(0, 0);
    ...
    glUniformMatrix3fv(m_shader->source, 1, GL_FALSE, toSource.getMatrix().data());
    ...
    glDrawArrays(GL_TRIANGLES, 0, 3);
    render.fbSize = oldFBSize;
    render.damage = oldDamage;
    g_pHyprRenderer->setProjectionType(oldProjection);
    m_pose->dirty = false;
    return {.framebuffer = out, .box = canvas};
}
} // namespace Hyprflip
