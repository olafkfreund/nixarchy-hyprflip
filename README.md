...
**Early release:** targets **the Hyprland main commit locked in [`flake.lock`](flake.lock)**
(moved nightly once it builds and passes the checks), with matching development headers
...
You need Hyprland **main at the commit locked in `flake.lock`**, its matching development headers, a matching
...
- **Version pin:** the Hyprland main commit in `flake.lock` only (0.56.2 is supported by
